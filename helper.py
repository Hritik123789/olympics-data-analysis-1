...
def data_nations_overtime(df,col):
    nations_over_time = df.drop_duplicates(['Year', col])['Year'].value_counts().reset_index().sort_values('Year',ascending=True)
    nations_over_time.rename(columns={'Year': 'Edition', 'count': col}, inplace=True)
    return nations_over_time
#***for modern python this throws an error tried to solve it but brain didnt brain.
...
#   x.rename(columns={'index':'Medal','Name_x':'Name'},inplace=True)
#   return x
# def most_successful(df,Sport):
#   temp_df=df.dropna(subset=['Medal'])
#   if Sport!='Overall':
...
# def most_successful_countrywise_analysis(df,country):
#   temp_df=df.dropna(subset=['Medal'])
